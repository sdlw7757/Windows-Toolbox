...
do {
    ...
    Write-Host "  [9] DLL修复工具 (运行库/DirectX/系统DLL)" -ForegroundColor Yellow
...
'@
        ...
        "9" {
            # 查找 DLLFixerScript.ps1
            $dllFixerPath = Join-Path $ScriptDir "DLLFixerScript.ps1"
            if (-not (Test-Path $dllFixerPath)) {
                $dllFixerPath = Join-Path $ScriptDir "DLL\DLLFixerScript.ps1"
            }
            if (-not (Test-Path $dllFixerPath)) {
                Write-Host "`n错误：未找到 DLLFixerScript.ps1 脚本。" -ForegroundColor Red
                Write-Host "请确保脚本位于根目录或根目录下的 DLL 文件夹中。" -ForegroundColor Yellow
                Read-Host "按 Enter 返回主菜单"
                break
            }
            ...
            do {
                ...
                Write-Host ""
                Write-Host "============================================================" -ForegroundColor Cyan
                Write-Host "                  DLL 修复工具子菜单                          " -ForegroundColor Green
                Write-Host "============================================================" -ForegroundColor Cyan
                Write-Host ""
                Write-Host "  [1] 全面体检 (扫描所有问题)"
                Write-Host "  [2] 运行库修复 (VC++ / .NET Runtime)"
                Write-Host "  [3] DirectX 修复"
                Write-Host "  [4] 系统 DLL 修复"
                Write-Host "  [5] 执行全部修复"
                Write-Host "  [0] 返回主菜单"
                Write-Host ""
                Write-Host "============================================================" -ForegroundColor Cyan
                $subChoice = Read-Host "请输入选项"

                switch ($subChoice) {
                    "1" {
                        Write-Host "`n正在运行全面体检..." -ForegroundColor Cyan
                        & $dllFixerPath -Mode scan -Auto
                        Read-Host "`n按 Enter 继续"
                    }
                    "2" {
                        Write-Host "`n正在运行运行库修复..." -ForegroundColor Cyan
                        & $dllFixerPath -Mode vcrepair -Auto
                        Read-Host "`n按 Enter 继续"
                    }
                    "3" {
                        Write-Host "`n正在运行 DirectX 修复..." -ForegroundColor Cyan
                        & $dllFixerPath -Mode dx -Auto
                        Read-Host "`n按 Enter 继续"
                    }
                    "4" {
                        Write-Host "`n正在运行系统 DLL 修复..." -ForegroundColor Cyan
                        & $dllFixerPath -Mode sysdll -Auto
                        Read-Host "`n按 Enter 继续"
                    }
                    "5" {
                        Write-Host "`n正在执行全部修复..." -ForegroundColor Cyan
                        & $dllFixerPath -Mode all -Auto
                        Read-Host "`n按 Enter 继续"
                    }
                    "0" {
                        break
